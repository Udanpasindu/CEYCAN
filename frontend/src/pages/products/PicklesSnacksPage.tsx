...
const PicklesSnacksPage = () => {
  ...
  const CATEGORY_NAME = "Pickles & Snacks";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCategory({
          name: CATEGORY_NAME,
          description: "Indulge in our traditional pickles and healthy snacks made with authentic Sri Lankan recipes.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <ProductPageLayout
      categoryName={category?.name || "Pickles & Snacks"}
      categoryIcon={Apple}
      description={category?.description || ""}
      heroImage={category?.image || "https://images.unsplash.com/photo-1599490659213-e2b9527bd087?w=600&h=400&fit=crop"}
      products={products}
    />
  );
};

export default PicklesSnacksPage;
